...
getDatas('/api/quote/tick?contract=btc.usdt&since=200000&until=300000').then(res=>{
    console.log('res',res)
})

async function getDatas(url){
    let fileList=urlConverter.getFilesList('/api/quote/tick?contract=btc.usdt&since=200000&until=300000')
    let requests=[]
    let uncachedReqs=[]
    ...
    for(let req of requests){
        if(req.cached) datas=datas.concat(req.datas)
        else{
